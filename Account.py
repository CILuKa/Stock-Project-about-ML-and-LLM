...
class Account:
    ...
    def sell_trigger(self, stock_id, day, all_df, index_df):
        ...
        if isinstance(all_df.iloc[0]['trade_date'], str):
            all_df['trade_date'] = all_df['trade_date'].str.replace("-", "", regex=False).astype(int)
    ...
    def update(self, day, all_df):
        ...
        for j in range(len(self.stock_id)):
            ...
            #print("asdasdas = ", all_df.iloc[0]['trade_date'])
            if isinstance(all_df.iloc[0]['trade_date'], str):
                all_df['trade_date'] = all_df['trade_date'].str.replace("-", "", regex=False).astype(int)
            idx = (all_df['trade_date'] == day) & (all_df['ts_code'] == self.stock_id[j])
            # print(all_df.loc[idx]['close'].shape)
            # print(day)
            # print(self.stock_id[j])
            # print(all_df['ts_code'][240748])
    ...
    def BackTest(self, buy_df, all_df, index_df, buy_price='close'):
        ...
        :return:
        """
        #print(index_df.iloc[0]['trade_date'])
        if isinstance(index_df.iloc[0]['trade_date'], str):
        ...
        for i in range(len(day_info)):
            day = day_info[i]
            if isinstance(buy_df.iloc[0]['trade_date'], str):
                buy_df['trade_date'] = buy_df['trade_date'].str.replace("-", "", regex=False).astype(int)
